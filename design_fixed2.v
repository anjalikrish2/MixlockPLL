// =============================================================================
// CANONICAL DESIGN FILE for this project (all_three_working_freqfix).
// mixlockadpll_topdesign_.v and mixlockadpll_topdesign_fixed.v are earlier
// iteration snapshots — do not edit them; they are archived/removed.
// This file carries both:
//   - the enrol_pulse/reco_pulse race-condition fix (see puf_top: both are
//     driven combinationally off puf_valid instead of a registered pulse
//     one cycle later, which used to miss fuzzy_extractor's IDLE transition)
//   - the structural Ring-Oscillator PUF (ro_puf) with $random(SIM_SEED)
//     per-cell delay skew, replacing the old DEVICE_ID-hashed sram_puf
// =============================================================================
...
// -----------------------------------------------------------------------
// CELLS matched oscillator pairs, one PUF response bit per pair. Each pair
// is built from two STRUCTURALLY IDENTICAL rings: a NAND start/stop gate
// followed by (STAGES-1) inverters, feeding back on itself. Both rings in
// a pair share the exact same topology and the exact same per-stage
// inverter delay (INV_DLY) — the only thing that differs between the two
// rings is a small NAND-gate delay skew, drawn once per cell at time 0 from
// $random(SIM_SEED) (see the initial block below) — independent of
// DEVICE_ID, which has no functional effect in this module and is kept
// only for interface compatibility with puf_top. A real RO PUF has no
// "device ID" input; its entropy comes from physically independent
// manufacturing variation. Here that's modeled by SIM_SEED: instantiate
// this module twice with two different SIM_SEED values to get two
// independent simulated "chips" (e.g. for a uniqueness/Hamming-distance
// test); the same SIM_SEED always reproduces the same chip. That per-cell
// randomness stands in for the unclonable manufacturing (Vt / wire /
// gate-oxide) variation that makes two "identical" silicon ring
// oscillators run at slightly different frequencies on a real chip — on
// the actual synthesizable Basys-3 RO PUF this project is modeled after,
// there is no injected delay at all; real place-and-route variation
// supplies it directly.
//
// On every `sample` pulse both rings in every pair are enabled for
// WIN_CYC clk cycles. Two free-running edge counters (count_a, count_b)
// tally how many times each ring toggled in that common window. Whichever
// ring toggled more is, by definition, the faster one — its count becomes
// count_fast, the other becomes count_slow — and the response bit is just
// "did ring A win the race". A handful of NOISE_BITS positions are then
// flipped per sample (same LFSR technique as before) to model the small
// fraction of near-tied pairs whose winner is not perfectly reproducible
// from one read to the next — real RO PUFs have this property too, which
// is exactly why the downstream fuzzy_extractor still carries BCH ECC.
// =============================================================================
`timescale 1ps/1ps
module ro_puf #(
    parameter [31:0] DEVICE_ID   = 32'hA5F3_1C2B,   // interface compat only — see note below, unused for RO timing
    parameter        CELLS        = 128,
    parameter        NOISE_BITS   = 3,
    parameter integer SIM_SEED    = 32'hC001_C0DE    // override per instance for distinct simulated "chips"
)(
    input  wire        clk,
    input  wire        rst_n,
    input  wire        sample,
    ...
    // ── Ring topology (identical for A and B, every pair) ───────────────────
    localparam integer STAGES    = 5;     // odd stage count -> free-running ring
    localparam integer COUNT_W   = 12;    // edge-counter width
    localparam integer WIN_CYC   = 4;     // clk cycles the rings run per sample
    localparam integer NAND_MIN  = 20;    // NAND delay range (ps-equivalent)
    localparam integer NAND_SPAN = 21;
    localparam real    SCALE_DIV = 1;     // 1 in this 1ps/1ps file, 1000.0 in a 1ns/1ps file
    localparam real    INV_DLY   = 8.0 / SCALE_DIV;   // shared inverter delay

    // ── Per-cell NAND delay skew: drawn from $random(SIM_SEED), NOT from
    //     DEVICE_ID. Seeding explicitly off SIM_SEED (instead of bare
    //     $random, which shares one implementation-defined global sequence
    //     across every instance in the simulation and gives you no control
    //     over whether two instances diverge) means two ro_puf instances
    //     given different SIM_SEED values are guaranteed independent
    //     "chips", and two given the same SIM_SEED are guaranteed identical
    //     — useful for a controlled uniqueness/Hamming-distance test. ──────
    reg [15:0] dly_a [0:CELLS-1];
    reg [15:0] dly_b [0:CELLS-1];
    integer    i_init;
    integer    seed_state;
    reg [31:0] r_init;   // unsigned: reinterpreting $random's signed result
                          // here keeps the '%' below always non-negative
    initial begin
        seed_state = SIM_SEED;
        for (i_init = 0; i_init < CELLS; i_init = i_init + 1) begin
            r_init = $random(seed_state);
            dly_a[i_init] = NAND_MIN + (r_init % NAND_SPAN);
            r_init = $random(seed_state);
            dly_b[i_init] = NAND_MIN + (r_init % NAND_SPAN);
        end
    end

    genvar p, s;
    wire [CELLS-1:0]   ro_a_out, ro_b_out;
    wire [CELLS-1:0]   clean_vec;
    reg  [COUNT_W-1:0] count_a [0:CELLS-1];
    reg  [COUNT_W-1:0] count_b [0:CELLS-1];

    reg ro_enable;

    generate
        for (p = 0; p < CELLS; p = p + 1) begin : RO_PAIR
            wire [STAGES-1:0] chain_a, chain_b;

            // Oscillator A — structural NAND-gated ring
            nand #(dly_a[p]/SCALE_DIV) g_nand_a (chain_a[0], ro_enable, chain_a[STAGES-1]);
            for (s = 1; s < STAGES; s = s + 1) begin : INV_A
                not #(INV_DLY) g_inv_a (chain_a[s], chain_a[s-1]);
            end

            // Oscillator B — same topology, independent instance
            nand #(dly_b[p]/SCALE_DIV) g_nand_b (chain_b[0], ro_enable, chain_b[STAGES-1]);
            for (s = 1; s < STAGES; s = s + 1) begin : INV_B
                not #(INV_DLY) g_inv_b (chain_b[s], chain_b[s-1]);
            end

            assign ro_a_out[p] = chain_a[STAGES-1];
            assign ro_b_out[p] = chain_b[STAGES-1];

            // Edge counters — cleared at the start of every sample window
            always @(posedge ro_a_out[p] or posedge sample or negedge rst_n)
                if (!rst_n || sample) count_a[p] <= {COUNT_W{1'b0}};
                else if (ro_enable)   count_a[p] <= count_a[p] + 1'b1;

            always @(posedge ro_b_out[p] or posedge sample or negedge rst_n)
                if (!rst_n || sample) count_b[p] <= {COUNT_W{1'b0}};
                else if (ro_enable)   count_b[p] <= count_b[p] + 1'b1;

            // count_fast/count_slow: counter-based comparison between the pair
            wire [COUNT_W-1:0] count_fast = (count_a[p] >= count_b[p]) ? count_a[p] : count_b[p];
            wire [COUNT_W-1:0] count_slow = (count_a[p] >= count_b[p]) ? count_b[p] : count_a[p];
            assign clean_vec[p] = (count_a[p] >= count_b[p]);   // 1 = ring A is count_fast
        end
    endgenerate

    // ── Window sequencer: enable both rings for WIN_CYC clk cycles ──────────
    reg [2:0] win_cnt;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ro_enable <= 1'b0; win_cnt <= 3'd0;
        end else if (sample) begin
            ro_enable <= 1'b1; win_cnt <= WIN_CYC;
        end else if (ro_enable) begin
            if (win_cnt > 3'd1) win_cnt <= win_cnt - 3'd1;
            else                ro_enable <= 1'b0;
        end
    end
    wire window_done = ro_enable && (win_cnt == 3'd1);

    // ── Latch the response, with a few noisy (near-tied) bits flipped ───────
    reg [7:0] noise_lfsr;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            noise_lfsr <= 8'hA3;
            ...
            if (window_done) begin
                begin : noise_gen
                    ...
                    lfsr_tmp = noise_lfsr;
                    mask     = 128'd0;
                    for (k = 0; k < NOISE_BITS; k = k + 1) begin
                        lfsr_tmp  = {lfsr_tmp[6:0], 1'b0} ^ (lfsr_tmp[7] ? 8'hB8 : 8'h00);
                        pos       = lfsr_tmp[6:0];
                        mask[pos] = 1'b1;
                    end
                    noise_lfsr <= lfsr_tmp;
                    puf_raw    <= clean_vec ^ mask;
                end
                puf_valid <= 1'b1;
            end
        end
...
module fuzzy_extractor (
    ...
    // Stability mask from the 30-read per-bit analysis (tb_bitstability.v, SIM_SEED=C001_C0DE):
    // bits 0,8,16,24,32,40,48,56,64,68,72,80,88,96,104,112,120,126 flipped across reads (18
    // unstable positions) -> STABLE_MASK clears those, keeping the 110 positions that never flipped.
    localparam [127:0] STABLE_MASK = ~(
        (128'd1 <<   0) | (128'd1 <<   8) | (128'd1 <<  16) | (128'd1 <<  24) |
        (128'd1 <<  32) | (128'd1 <<  40) | (128'd1 <<  48) | (128'd1 <<  56) |
        (128'd1 <<  64) | (128'd1 <<  68) | (128'd1 <<  72) | (128'd1 <<  80) |
        (128'd1 <<  88) | (128'd1 <<  96) | (128'd1 << 104) | (128'd1 << 112) |
        (128'd1 << 120) | (128'd1 << 126));
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            state <= IDLE; helper_data <= 128'd0; mask_data <= 128'd0;
            ...
            case (state)
                ...
                S_ENROL: begin
                    ...
                    mask_data   <= STABLE_MASK;
                    K_puf <= lfsr_hash(noisy_word & STABLE_MASK);
                    puf_ready <= 1'b1;
                    state <= DONE;
                end
                RECO: begin
                    begin : rb reg [127:0] c; c = bch_syndrome(noisy_word & mask_data, helper_data & mask_data);
...
module puf_top #(
    ...
    parameter integer SIM_SEED      = 32'hC001_C0DE   // pass-through to ro_puf; override for a distinct simulated chip
)(
    input  wire        clk,
    input  wire        rst_n,
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (!seq_done) begin
                ...
                else if (settle_cnt == SETTLE_CYCLES) begin sample_pulse <= 1'b1; settle_cnt <= settle_cnt + 1; end
                else if (puf_valid) begin seq_done <= 1'b1; end
            end
        end
    end

    ro_puf #(.DEVICE_ID(DEVICE_ID),.CELLS(128),.NOISE_BITS(3),.SIM_SEED(SIM_SEED)) u_puf (
...
// -------------------
//   BLOCK A — puf_top
//     Ring-Oscillator PUF (128 matched pairs) + Fuzzy Extractor (BCH ECC + LFSR hash)
...
module mixlock_adpll_top #(
    parameter [31:0]  DEVICE_ID      = 32'hA5F3_1C2B,
    parameter         SETTLE_CYCLES  = 16,
    parameter integer SIM_SEED       = 32'hC001_C0DE   // pass-through to puf_top/ro_puf; override for a distinct simulated chip
)(
    // System
...
// =============================================================================
// Module  : disorc_checker
// Project : Adaptive Hardware Security — MixLock ADPLL
//
// Purpose : Real-RTL tamper-detection bounds checker for DisORC, split into
//           two INDEPENDENT fault flags so the source of a fault is never
//           ambiguous — they are deliberately not OR'd together here.
//
//   digital_fault — divider-side check. Through normal trll_locked_M
//     operation, M_locked can only be M_prog (correct key) or
//     (scramble % 6) + 2, which is structurally guaranteed to be in [2,7]
//     (wrong key) — see trll_locked_M. M_locked == 0 or 1 while wrong_key
//     is asserted cannot happen through that logic, so it can only mean
//     the divider input was tampered with directly (fault injection
//     bypassing TRLL). Registered on clk, sticky until rst_n.
//
//   analog_fault  — DCO-side check. Measures OUT_CLK frequency by counting
//     edges (OUT_CLK domain, free-running counter) within a window timed
//     by REF_CLK, and compares the count against thresholds derived from
//     [MIN_FREQ_HZ, MAX_FREQ_HZ] — the DCO's measured tuning range from
//     tb_dco_characterize.v (~171 MHz .. ~1550 MHz). A window count outside
//     that band means OUT_CLK is running somewhere none of the DCO's 129
//     valid control codes can produce — an analog-domain fault, not a
//     key/divider issue. Toggle-synchronized across the OUT_CLK/REF_CLK
//     boundary. Registered on REF_CLK, sticky until rst_n.
// =============================================================================
`timescale 1ps/1ps
module disorc_checker #(
    parameter integer MIN_FREQ_HZ       = 171_000_000,
    parameter integer MAX_FREQ_HZ       = 1_550_000_000,
    parameter integer REF_CLK_FREQ_HZ   = 100_000_000,  // frequency of REF_CLK -> sets the measurement timebase
    parameter integer WINDOW_REF_CYCLES = 10             // OUT_CLK edges counted per this many REF_CLK cycles
)(
    input  wire       clk,           // digital domain clock (system / DisORC clk)
    input  wire       rst_n,

    // ── Divider-side (digital) check ────────────────────────────────────────
    input  wire       wrong_key,      // from trll_locked_M: |key_diff
    input  wire [2:0] M_locked,
    output reg        digital_fault,

    // ── DCO-side (analog / frequency) check ─────────────────────────────────
    input  wire       REF_CLK,        // known-good reference / timebase clock
    input  wire       OUT_CLK,        // DCO output under test
    output reg        analog_fault
);

    // ── Divider-side check ──────────────────────────────────────────────────
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) digital_fault <= 1'b0;
        else if (wrong_key && (M_locked == 3'd0 || M_locked == 3'd1))
            digital_fault <= 1'b1;   // sticky — retains otherwise
    end

    // ── DCO-side check ──────────────────────────────────────────────────────
    // Edge-count thresholds for one WINDOW_REF_CYCLES-long REF_CLK window.
    // Division-before-multiplication ordering avoids 32-bit integer overflow
    // on MAX_FREQ_HZ * WINDOW_REF_CYCLES.
    localparam integer MIN_COUNT = MIN_FREQ_HZ / (REF_CLK_FREQ_HZ / WINDOW_REF_CYCLES);
    localparam integer MAX_COUNT = MAX_FREQ_HZ / (REF_CLK_FREQ_HZ / WINDOW_REF_CYCLES);

    // REF_CLK domain: toggle window_toggle every WINDOW_REF_CYCLES REF_CLK cycles
    reg [31:0] refcyc_cnt;
    reg        window_toggle;
    always @(posedge REF_CLK or negedge rst_n) begin
        if (!rst_n) begin
            refcyc_cnt    <= 32'd0;
            window_toggle <= 1'b0;
        end else if (refcyc_cnt == WINDOW_REF_CYCLES - 1) begin
            refcyc_cnt    <= 32'd0;
            window_toggle <= ~window_toggle;
        end else begin
            refcyc_cnt <= refcyc_cnt + 1'b1;
        end
    end

    // OUT_CLK domain: synchronize window_toggle (2-FF), detect toggle edges as
    // window boundaries, free-run an edge counter, snapshot it at each boundary.
    reg        wt_meta, wt_sync, wt_sync_d;
    reg [31:0] edge_count, latched_count;
    always @(posedge OUT_CLK or negedge rst_n) begin
        if (!rst_n) begin
            wt_meta <= 1'b0; wt_sync <= 1'b0; wt_sync_d <= 1'b0;
            edge_count <= 32'd0; latched_count <= 32'd0;
        end else begin
            wt_meta   <= window_toggle;
            wt_sync   <= wt_meta;
            wt_sync_d <= wt_sync;
            if (wt_sync != wt_sync_d) begin
                latched_count <= edge_count;
                edge_count    <= 32'd1;
            end else begin
    ...
    // Resync latched_count into REF_CLK domain (stable for a full window ->
    // ample margin for this 2-stage resync) and compare against bounds.
    reg [31:0] count_sync0, count_sync1;
    always @(posedge REF_CLK or negedge rst_n) begin
        if (!rst_n) begin
            count_sync0 <= 32'd0; count_sync1 <= 32'd0;
        end else begin
            count_sync0 <= latched_count;
            count_sync1 <= count_sync0;
        end
    end

    always @(posedge REF_CLK or negedge rst_n) begin
        if (!rst_n) analog_fault <= 1'b0;
        else if (count_sync1 != 0 && (count_sync1 < MIN_COUNT || count_sync1 > MAX_COUNT))
            analog_fault <= 1'b1;   // sticky — retains otherwise
    end
